...
def main() -> int:
    parser = argparse.ArgumentParser(description="Prepare or run Notion sync for inventory records.")
    parser.add_argument("--inventory", default="data/media_inventory.json", help="Input inventory JSON.")
    parser.add_argument("--output", default="exports/notion_sync_payload.json", help="Dry-run payload output.")
    ...
    payload = build_sync_payload(records, "notion")
    write_json(payload, Path(args.output))
    print(f"Prepared Notion sync payload for {len(records)} records: {args.output}")
    return 0
